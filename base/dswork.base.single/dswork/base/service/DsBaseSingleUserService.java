...
public class DsBaseSingleUserService extends BaseService<DsBaseUser, Long>
{
	@Autowired
	...
	@Override
	public int save(DsBaseUser po)
	{
		DsBaseOrg org = new DsBaseOrg();
		org.setId(po.getId());
		org.setPid(po.getOrgid());
	...
	@Override
	public int update(DsBaseUser po)
	{
		DsBaseOrg org = (DsBaseOrg) orgDao.get(po.getId());
		if(!org.getName().equals(po.getName()))
		{
			org.setName(po.getName());
			orgDao.update(org);
		}
		if(org.getPid().longValue() != po.getOrgid().longValue())
		{
			orgDao.updatePid(po.getId(), po.getOrgid());
		}
		dao.update(po);
		return 1;
	}
